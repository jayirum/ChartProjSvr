...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	if (m_Dup.CheckDuplicate("ChartMakerUI") == -1)
	{
		AfxMessageBox("<ChartMakerUI>이미 프로세스가 실행중입니다.");
...
BOOL CESApiDllDlgDlg::IR_LoadSiseSymbol()
{
	...
	while (db->IsNextRow())
	{
		...
		ST_CHART_THREAD* p = new ST_CHART_THREAD;
		db->GetStr("ARTC_CODE", zTemp);
		ir_cvtcode_uro_6e(zTemp, p->zArtcCode);
		m_mapSymbol[symbol] = p;
		IR_PrintMsg(LOGTP_SUCC, TRUE, "[%s][%s] 차트구성종목", p->zArtcCode, symbol.GetBuffer());
...
LRESULT CESApiDllDlgDlg::OnRecvData(WPARAM wParam, LPARAM lParam)
{
	...
	case AUTO_0933:// 종목 시세 실시간
	{
		...
		std::map<CString, ST_CHART_THREAD*>::iterator it = m_mapSymbol.find(sShortCode);
		...
		else
		{
			...
			pData->seq = ++m_siseSeq;
			ST_CHART_THREAD* p = (*it).second;
			PostThreadMessage(p->pChart->GetMyThreadID(), WM_CHART_DATA, 0, (LPARAM)pData);

			//// DB 저장을 위해
			PostThreadMessage(m_unSaveThread, WM_SISE_RECV, 0, (LPARAM)pData);
		}
	}break;
	default:
		break;
