...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	m_pTcpSvr = new CTcpSrv();
	if (!m_pTcpSvr->Begin(atoi(port), zLocalIP))
	{
		AfxMessageBox("TCP  초기화실패");
		return FALSE;
	}

	g_log.log(LOGTP_SUCC, "TCP송신 초기화 성공(LOCAL:%s)(PORT:%s)", zLocalIP, port);
...
unsigned WINAPI CESApiDllDlgDlg::SendPrcThread(LPVOID lp)
{
	...
	int size = sizeof(ST_PACK2CHART_EX);
	sprintf(zSize, "%4d", size);
	char sendMsg[1024];
	while (p->m_bContinue)
	{
		Sleep(1);
		...
		while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			case WM_SISE_RECV:
				...
				ST_PACK2CHART_EX* pSend = (ST_PACK2CHART_EX*)sendMsg;
				memset(pSend, 0x20, sizeof(ST_PACK2CHART_EX));
				pSend->STX[0] = 0x02;
				memcpy(pSend->Len, zSize, strlen(zSize));
				...
				*(sendMsg + size) = 0x00;
				...
				//int nLen = p->m_pTcpSvr->SendData(sendMsg, size);
				//if (nLen < 0) {
				//	g_log.log(LOGTP_ERR, "[SEND ERROR:%s]", p->m_pTcpSvr->GetMsg());
				//}
				p->m_pTcpSvr->SendDataAllClient(sendMsg, size);
				//else
