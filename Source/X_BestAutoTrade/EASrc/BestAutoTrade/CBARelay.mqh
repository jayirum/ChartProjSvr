...
private:
    ...
    CHANNEL_INFO    m_channelToMaster;
...
{
    if(!m_channelToMaster.bInit)
        return Q_SUCCESS;
  
    int ret = nn_recv (m_channelToMaster.sock, RecvBuff, nBuffLen, 0 );
...
{
    return CharArrayToString (m_channelToMaster.chName);
}

bool CBARelay::InitSlave()
{
    m_channelToMaster.sock = nn_socket(AF_SP, NN_SUB);    
    if (m_channelToMaster.sock == -1) {
		m_msg = StringFormat( "Slave Channel create Failed:%s", nn_err_strerror(nn_errno()));
		return false;
	}
	
   int val=0;   
   int ret = nn_setsockopt(m_channelToMaster.sock, NN_SUB, NN_SUB_SUBSCRIBE, val, 0);
   if (ret == -1) {
	...
	StringToCharArray(sChannelNm, m_channelToMaster.chName);
	
	m_channelToMaster.endpoint = nn_connect(m_channelToMaster.sock, m_channelToMaster.chName);
	if (m_channelToMaster.endpoint < 0){
		m_msg = StringFormat( "Connect Master Channel Failed:%s", nn_err_strerror(nn_errno()));
		return false;
	}
   
	int nTimeout = TIMEOUT_MS_RECV;
   ret = nn_setsockopt(m_channelToMaster.sock, NN_SOL_SOCKET, NN_RCVTIMEO, nTimeout, sizeof(nTimeout));
	if (ret == -1) {
...
bool CBARelay::RegisterAsSlave(string sMasterAcc)
{
    return RegUnregSlave(sMasterAcc,TP_REG);
}


bool CBARelay::UnRegisterAsSlave(string sMasterAcc)
{
    return RegUnregSlave(sMasterAcc, TP_UNREG);
}

bool CBARelay::RegUnregSlave(string sMasterAcc, string sRegUnreg)
{
...
{
   if(m_chToSrv.sock>0){
      nn_shutdown(m_chToSrv.sock, m_chToSrv.endpoint);
      nn_close(m_chToSrv.sock);
      m_chToSrv.sock = -1;
   }
   if(m_channelToMaster.sock>0){
      nn_shutdown(m_channelToMaster.sock, m_channelToMaster.endpoint);
      nn_close(m_channelToMaster.sock);
      m_channelToMaster.sock = -1;
   }
}
...
bool CBARelay::Initialize()
{
	...
	m_chToSrv.endpoint = nn_connect(m_chToSrv.sock, m_chToSrv.chName);
	if (m_chToSrv.endpoint < 0){
		m_msg = StringFormat( "Connect Server Channel Failed:%s", nn_err_strerror(nn_errno()));
		return false;
	}

	int nTimeout = TIMEOUT_MS_SEND;
   int ret = nn_setsockopt(m_chToSrv.sock, NN_SOL_SOCKET, NN_SNDTIMEO, nTimeout, sizeof(nTimeout));
	if (ret == -1) {
