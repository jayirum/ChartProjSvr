...
void COrdManager::Clear(bool bMapLast)
{
	...
	else
	{
		if (m_mapOrdNew.size() > 0) {
			...
			m_mapOrdNew.clear();
			//g_log.Log("[BAUTILS][Clear]New Map");
		}
	}
}
...
void COrdManager::SetComplete()
{
	...
	for (it = m_mapOrdNew.begin(); it != m_mapOrdNew.end(); it++)
	{
		...
		//g_log.Log("[BAUTILS][SetComplete] (Copy to Last Map)(Ticket:%d)", (*it).first);
		//nCnt++;
	}
	...
	//if (nCnt > 0)
	//{
	//	g_log.Log("[BAUTILS][SetComplete]Last size:%d, New Size:%d", m_mapOrdLast.size(), m_mapOrdNew.size());
	//	g_log.Log("[BAUTILS]===================================================");
	//}
}
...
)
{
	...
	map<int, MT4_ORD*>::iterator itLast = m_mapOrdLast.begin();
	MT4_ORD* p = (MT4_ORD*)(*itLast).second;

	ticket	= (*itLast).first;
	lots	= p->lots;

	map<int, MT4_ORD*>::iterator itNew = m_mapOrdNew.find(ticket);

	// exists. Meaning not closed.
	if (itNew != m_mapOrdNew.end())
	{
		g_log.Log("[BAUTILS][GetClosedOrd](OpenedTicket:%d)", ticket);
		ticket = -1;
	}
	else
	{
		g_log.Log("[BAUTILS][DELETED](OpenedTicket:%d)(Type:%d)(OpenPrc:%f)(OpenLots:%f)(OpenTM:%d)",
			ticket, p->type, p->open_time, p->lots, p->open_time);
	}
