...
unsigned WINAPI CAbotMain::Thread_Monitor(LPVOID lp)
{
	...
	while (pThis->m_bContinue)
	{
		...
		if (nLen < 0)
		{
			g_log.log(ERR, "TICK DATA PAKCET 이상(%s)(%s)", pBuf, pThis->m_pMonitorClient->GetMsg());
			pThis->m_pMemPool->release(pBuf);
			continue;
		}
...
unsigned WINAPI CAbotMain::Thread_ApiTick(LPVOID lp)
{
	...
	while (p->m_bContinue)
	{
		...
		if (nLen < 0)
		{
			g_log.log(ERR,"TICK DATA PAKCET 이상(%s)(%s)", pBuf, p->m_pApiClient[API_TICK]->GetMsg());
			p->m_pMemPool->release(pBuf);
			continue;
		}
...
unsigned WINAPI CAbotMain::Thread_ApiOrd(LPVOID lp)
{
	...
	while (p->m_bContinue)
	{
		...
		if (nLen < 0)
		{
			g_log.log(ERR, "API_ORD DATA PAKCET 이상(%s)(%s)", pBuf, p->m_pApiClient[API_ORD]->GetMsg());
			p->m_pMemPool->release(pBuf);
			continue;
		}
		...
		else
		{
			...
			if (it == p->m_mapStrat.end())
			{
				p->unlock();
				p->m_pMemPool->release(pBuf);
				g_log.log(ERR, "[%s] 종목은 요청한 종목이 아니다.", sSymbol.c_str());
				continue;
			}
			p->unlock();
		}
...
VOID CAbotMain::ApiOrd_Err(char* pPacket)
{
	...
	g_log.log(INFO, "[주문오류](API CODE:%.*s) MSG(%.*s) UUID(%.*s)",
...
VOID CAbotMain::ApiOrd_RealOrd(char* pPacket)
{
	...
	//g_log.log(INFO, "[주문REAL수신](%.*s) No(%.*s) Side(%c) Prc(%.*s) Qty(%.*s) Rmnq(%.*s) DT(%.8s) TM(%.9s)",
	//	sizeof(p->Symbol), p->Symbol,
	//	sizeof(p->OrdNo), p->OrdNo,
	//	p->Side[0],
...
VOID CAbotMain::ApiOrd_RealCntr(char* pPacket)
{
	...
	g_log.log(INFO, "[체결REAL수신](%.*s) No(%.*s) Side(%c) OPrc(%.*s) CPrc(%.*s)"
