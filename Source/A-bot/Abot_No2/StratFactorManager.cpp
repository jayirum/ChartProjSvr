...
CStratFactorManager::CStratFactorManager(char* pzSymbol)
{
...
CStratFactorManager::~CStratFactorManager()
{
	DeleteCriticalSection(&m_cs);
}

VOID CStratFactorManager::SetOpenPrc(char *pzOpenPrc)
{
	//strcpy(m_param.zOpenPrc, pzOpenPrc);
}


BOOL CStratFactorManager::LoadStratInfo()
{
	...
	char zQ[1024];
	sprintf(zQ, "EXEC ABOT_NO2_LOAD_SYMBOL_INDI '%s' ", m_symbol.symbol());
	...
	else
	{
		if (db->IsNextRow())
		{
			...
			m_param.nAcmlLossTickCap = db->GetLong("ACML_LOSSTICK_CAP");
			m_param.nAcmlPTTickCap = db->GetLong("ACML_PTTICK_CAP");
...
//
//void CStratFactorManager::SetInitInfo(double dTickVal, double dTickSize, int nDotCnt,
...
BOOL			bFinish;
*/
void CStratFactorManager::AcptEntryNewProc(LPCSTR lpCntrBsTp, LPCSTR lpCntrPrc, LPCSTR lpCntrQty)
{
...
void CStratFactorManager::AcptEntryAddProc(LPCSTR lpCntrBsTp, LPCSTR lpCntrPrc, LPCSTR lpCntrQty)
{
	double dCntrQty = atof(lpCntrQty);
...
void CStratFactorManager::AcptCntrProc(LPCSTR lpCntrBsTp, LPCSTR lpCntrPrc, LPCSTR lpCntrQty)
{
	lock();
...
void CStratFactorManager::AcptCloseProc(LPCSTR lpCntrBsTp, LPCSTR lpCntrPrc, LPCSTR lpCntrQty)
{
	double dCntrQty = atof(lpCntrQty);
	double dPLQty;
	double dPrcGap;
	...
	if (m_pos.dQty == dCntrQty)
	{
		...
		//}
		if (m_pos.nAcmlLossTickCap >= m_param.nAcmlLossTickCap)
		{
			m_pos.bFinish = TRUE;
...
double	CStratFactorManager::GetPTLevelTouched()
{
...
/*
	// 익절 자리 touch 하면 주문
*/
BOOL CStratFactorManager::IsProfitTakingCondition(
...
)
{
	...
	double dPrcMax = 0;	// atof(m_pos.zMaxPLPrc);
...
//VOID CStratFactorManager::SetConfigInfo(LPCSTR pzOpenPrc, LPCSTR pzOrdQty, LPCSTR pzEndTM, LPCSTR sMaxSL,
...
VOID CStratFactorManager::SetOrderSent(double dSentQty)
{
...
void CStratFactorManager::SetMaxPLPrc(char* pzCurrPrc)
{
	if (m_pos.Status == FLAG_NONE)
		return;
	...
	double dTouchPrc = 0;
	double dOpenPrc = 0;// atof(m_param.zOpenPrc);
...
//
//*/
//BOOL CStratFactorManager::IsPTCondition(char* pzCurrPrc, _Out_ char* pMsg, _Out_ char* pDbLog)
//{
//	BOOL bResult = FALSE;
