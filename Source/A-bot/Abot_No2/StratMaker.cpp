...
//
////////////////////////////////////////////////////////////////////////////////////////////////////////
CStratMaker::CStratMaker(char* pzSymbol, char* pzArtc, CStratFactorManager* f) :CBaseThread("SratMaker")
{
	m_option = NULL;
	m_chart = NULL;
...
BOOL CStratMaker::Initialize()
{
	...
	// 시가일치전략
	m_stratOpenMatched = new CStrat_OpenPrcMatched(m_f);
	if (!m_stratOpenMatched->Initialize())
	{
		//TODO. NOTI
		return FALSE;
	}
...
CStratMaker::~CStratMaker()
{
	...
	SAFE_DELETE(m_stratOpenMatched);
...
VOID CStratMaker::StratProc(char* pMarketData)
{
	...
	// 아직 주문 진행중이면 패스
	if (m_f->IsAlreadySent())
		return;


	// 시가일치 전략
	m_stratOpenMatched->MainProc();
