...
unsigned WINAPI CAbotMain::Thread_ApiTick(LPVOID lp)
{
	...
	while (p->m_bContinue)
	{
		...
		{
			p->showMsg(FALSE, "30 Minitues passed without receiving any market data");
		}
	...
	else
	{
		while (db->IsNextRow())
		{
			...
			if (bCreateStrat)
			{
				...
				st->f = new CStratFactorManager(zSymbol);
