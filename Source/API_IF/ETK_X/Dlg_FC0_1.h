#if !defined(AFX_DLG_FC0_H__02C7D1E4_2C21_46E3_891F_7256BFB82BF2__INCLUDED_)
#define AFX_DLG_FC0_H__02C7D1E4_2C21_46E3_891F_7256BFB82BF2__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000
// Dlg_FC0.h : header file
//

/////////////////////////////////////////////////////////////////////////////
// CDlg_FC0_1 dialog


/***************************************************************************


  
		KS200 선물, 옵션의 시세, 호가를 모두 요청한다.



***************************************************************************/

#include "../../include/common.h"
#include "../../include/Packet.h"
#include "../../include/UtilLibInc.h"


//시세중계
#include "../../libsrc/SiseClntSend.h"


#define ETK_REAL_SISE_F			"FC0"
#define ETK_REAL_SISE_O			"OC0"
#define ETK_REAL_HOGA_F			"FH0"
#define ETK_REAL_HOGA_O			"OH0"

#define ETK_REAL_HOGA_FU		"OVH"	// 해외선물호가
#define ETK_REAL_SISE_FU		"OVC"	// 해외선물시세

//CME
#define ETK_REAL_SISE_F_CME		"NC0"
#define ETK_REAL_HOGA_F_CME		"NH0"

#define MSG_RE_CONNECT		WM_USER + 889
//TCPRELAY
#define WM_SISE_DATA	WM_USER + 195

#define DEF_TARGET_CNT	1

class CDlg_FC0_1 : public CDialog
{
	DECLARE_DYNCREATE( CDlg_FC0_1 )
// Construction
public:
	CDlg_FC0_1(CWnd* pParent = NULL);   // standard constructor

	//////////////////////////////////////////////////////////////////////////
	//	USER FUNCTION
	BOOL	SockInit();
	VOID	SockClose();
	BOOL	GetStkCodeList_FU();
	BOOL	GetNowPrc( CString i_sStkCode );
	VOID	Send_Sise_KSF( char* psOrgData );
	VOID	Send_Sise_KSO( char* psOrgData );
	VOID	Send_Hoga_KSF( char* psOrgData );
	VOID	Send_Hoga_KSO( char* psOrgData );
	
	//////////////////////////////////////////////////////////////////////////
	//static	DWORD WINAPI	ConnectThread(LPVOID lp);
	//static	DWORD WINAPI	SendThread_SHM(LPVOID lp);

	//CUdpSocket	*m_sockM;
	//CTcpSock	*m_pSock	[DEF_TARGET_CNT];
	DWORD		m_dwThreadID;
	HANDLE		m_hThread	;
	HANDLE		m_hDie		;
	int			m_nUdpPortLC	;
	int			m_nUdpPortReal	;


	///////////////////////////////////////////////////////////////////////////
	//	시세를 SHM 에 직접 저장
	VOID SaveSHM( char* pSise );

	//시세중계
	//TCPRELAY CSiseClntSend	*m_clntSend;

	///////////////////////////////////////////////////////////////////////////////
	//	국내시세 처리
	VOID	SiseProc_KS(LPRECV_REAL_PACKET pRealPacket);
	VOID	SaveSHM_KS(char* pSise);
	BOOL	InitSHM_KS();
	BOOL	GetStkCodeList_KS();
	CSiseClntSend	*m_clntSend_KS;

// Dialog Data
	//{{AFX_DATA(CDlg_FC0_1)
	enum { IDD = IDD_FC0_1 };
	CListBox	m_lstMsg;
	//}}AFX_DATA


// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CDlg_FC0_1)
	protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL

// Implementation
private:
	
	CLog	m_logF, m_logInner;

	CUdpSend	*m_sockFront;	//	FRONT SERVER 에게 전송할 UDP SOCKET
	CUdpSend	*m_sockLosscut;	//	LOSSCUT 에게 전송할 UDP SOCKET

	ScrewDB		m_db;
	
	char szPath[_MAX_PATH];
	char		m_szMcastIP[32];
	char		m_szLocalIP[32];
	int			m_nPortFront;
	int			m_nPortFront_O;
	int			m_nPortSHM;
	int			m_nPortMT;
	list<CString>		m_sStkList;
	map<CString, CString>	m_mapNowPrc;	//	종목별 현재가 저장

	
	CSharedMem		m_shm, m_shmKS;

	HANDLE					m_hThdDummySise;				// skeo 2011-01-17
	static DWORD WINAPI		ProcDummySiseTest(LPVOID lp);	// skeo 2011-01-17

	//SMILOR.2스레드.
	static	DWORD WINAPI SiseThread(LPVOID lp);
	VOID	SiseProc(LPRECV_REAL_PACKET p);
	HANDLE				m_hSiseThr;
	CRITICAL_SECTION	m_csSise;

	//////////////////////////////////////////////////////////////////////////////
	//TCPRELAY
	BOOL	Relay_Begin();
	VOID	Relay_End();
	static	DWORD WINAPI  Relay_SendDThread(LPVOID lp);
	VOID	Relay_PostData(LPSTR lpData, INT nLen);
	VOID	Relay_SendData(LPSTR lpData, INT nLen);
	BOOL	Relay_RequestSendIO( SOCKET sock, char* pSendBuf, int nSendLen );

	SOCKET		m_sockListen;
	SOCKADDR_IN	m_localsin;
	WSAEVENT	m_wsaAccept;				//	accept event
	WSAEVENT	m_wsaSend;
	set<SOCKET>	m_setRelaySock;
	CRITICAL_SECTION	m_csRelaySock;
	BOOL				m_bRelayContinue;
	HANDLE				m_hRelayThread;
	DWORD				m_dwRelayThread;

	list<CString>		m_lstRelayData;
	CRITICAL_SECTION	m_csRelayData;

	//////////////////////////////////////////////////////////////////////////////

protected:
	CString				m_strCode, m_strCode_H;
	void				InitCtrls();
	void				AdviseData(char i_cSiseYN, CString sStk);
	void				UnadviseData(char i_cSiseYN, CString sStk);

	// Generated message map functions
	//{{AFX_MSG(CDlg_FC0_1)
	virtual BOOL OnInitDialog();
	afx_msg void OnButtonUnadvise();
	afx_msg void OnButtonTest_1();
	//}}AFX_MSG
	afx_msg void		OnDestroy();
	afx_msg void		OnButtonRequest		();
	afx_msg	LRESULT		OnXMReceiveRealData	( WPARAM wParam, LPARAM lParam );
	DECLARE_MESSAGE_MAP()
};

//{{AFX_INSERT_LOCATION}}
// Microsoft Visual C++ will insert additional declarations immediately before the previous line.

#endif // !defined(AFX_DLG_FC0_H__02C7D1E4_2C21_46E3_891F_7256BFB82BF2__INCLUDED_)
