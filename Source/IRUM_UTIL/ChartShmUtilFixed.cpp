...
CChartShmUtilFixed::CChartShmUtilFixed(int nChartNameTp)
{
	m_pShm = NULL;
	m_bOpen = FALSE;
...
CChartShmUtilFixed::~CChartShmUtilFixed()
{
	m_bOpen = FALSE;
}
...
// 120분 차트는 6시 부터 시작한다
char* CChartShmUtilFixed::ComposeChartName(char* date, char* time, int tp, char* out)
{
	if (tp == TP_1MIN)
...
// date : yyyymmdd
// time : hh:mm:ss
char* CChartShmUtilFixed::ComposeChartName_1min(char* date, char* time, char* out)
{
	char zHour[32], zMin[32], zSec[32];
	char zTm[32];
	sprintf(zTm, "%.8s", time);
...
BOOL CChartShmUtilFixed::OpenChart(char* pzArtc)
{
	...
	m_pShm = new CSharedMem();
	if (!m_pShm->Open((LPCTSTR)m_zShmNm, (LPCTSTR)m_zMutexNm))
	{
		sprintf(m_zMsg, "CHART SHM OPEN 에러(%s)(artc:%s)(%s)",m_zShmNm, pzArtc, m_pShm->GetErr());
		return FALSE;
	}
	m_bOpen = TRUE;
	return TRUE;
...
VOID CChartShmUtilFixed::ReleaseChart()
{
	if (m_pShm)
	{
		m_pShm->Close();
...
// ORG
//BOOL CChartShmUtilFixed::GetChartData(char *pzSymbol, CHART_TP ChartTp, char* pzChartNm,
...
/*
	...
	time : hh:mm:ss
*/
BOOL CChartShmUtilFixed::CurrChart(char *pzSymbol, CHART_TP ChartTp, char* date, char *time, 
	_Out_ ST_SHM_CHART_UNIT& chart)
{
	if (!m_bOpen)
	{
...
/*
	가장 최근 차트 데이터 반환
*/
BOOL CChartShmUtilFixed::GetLastChartData(char *pzSymbol, CHART_TP ChartTp, _Out_ ST_SHM_CHART_UNIT* pChart)
{
	if (!m_bOpen)
	{
...
BOOL CChartShmUtilFixed::IsSavedEnoughDataCnt(_In_ char* pGroupKey, _In_ int nBaseCnt)
{
	if (!m_pShm || !pGroupKey) return FALSE;

	int nCnt = m_pShm->GetCurrStructCnt(pGroupKey);
	return (nCnt >= nBaseCnt);
}

BOOL	CChartShmUtilFixed::CompareHiLo(char *pzSymbol, CHART_TP ChartTp, char* pzCurrPrc, int nCompCnt, 
	...
	_Out_ ST_SHM_CHART_UNIT* currChart)
{
	...
	int nLoop = 0;
	while (FALSE == m_pShm->DataGet(zGroupKey, zChartNm, (char*)currChart, &nCode, zErr))
	{
		Sleep(10);
		if (++nLoop > 10) {
			// There is no chart of the current time. Something is wrong.
	...
	*pnHiCnt = *pnLoCnt = 0;
	for(int i=0;i<nCompCnt-1;i++)
	{
		...
		if (!m_pShm->DataGet(zGroupKey, zChartNm, (char*)&chart, &nCode, zErr))
			break;
	}
	return TRUE;
}

BOOL CChartShmUtilFixed::PrevChart(char *pzSymbol, CHART_TP ChartTp,
	_In_ ST_SHM_CHART_UNIT& currChart, _Out_ ST_SHM_CHART_UNIT& prevChart)
{
	...
	BOOL bExist = m_pShm->DataGet(zGroupKey, currChart.prevNm, (char*)&prevChart, &nCode, zErr);
	...
	while (FALSE == bExist)
	{
		...
		bExist = m_pShm->DataGet(zGroupKey, currChart.prevNm, (char*)&prevChart, &nCode, zErr);
...
CROSS_TP CChartShmUtilFixed::GetCross(
	char* pzSymbol, 
	CHART_TP chartTp, 
	ST_SHM_CHART_UNIT* chart, 
	int nDotCnt, 
...
)
{
	...
	*pzLMALastPrc = 0x00;
	...
	int nCode;
	char zErr[512];
	BOOL bExist = m_pShm->DataGet(zGroupKey, zStartChartNm, (char*)&firstChart, &nCode, zErr);
...
*/
BOOL CChartShmUtilFixed::GetPreviousChartNm(_In_ char* pzOrgChartNm, _In_ CHART_TP ChartTp, _In_ int nPreviousCnt, _Out_ char* pResult)
{
	int nValCnt;
	switch (ChartTp)
	{
