...
const int SCREEN_TERMINAL_X_SIZE = 240;
const int SCREEN_TERMINAL_Y_SIZE = 60;
...
public:
	...
	{
		//todo: precision depends on channel requested -> have to be manageable
		char szData[1024];
		sprintf(szData, "%.5f,%d,%.5f", m_dPrice, m_nCount, m_dAmount); 
...
public:
	...
	void ShowBookBidData()
	{
		...
		{
			//todo:convert method for logging into file
			char szDateTime[15] = "\0";
...
public:
	...
	char *CandleDataToCharString()
	{
		//todo: precision depends on channel requested -> have to be manageable
		char szData[1024];
		sprintf(szData, "%.5f,%.5f,%.5f,%.5f,%.5f", dOpen, dClose, dHigh, dLow, dVolume); 
		return szData;
	}
