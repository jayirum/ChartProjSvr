...
BOOL CCreateSaveShm::Create()
{
	...
	else if (nRet == 0)
	{
		...
		//printf("이미 SHM 이 생성되어 있어서 OPEN 한다.(%s)(%s)", m_zShmNm, m_zMutexNm);
...
BOOL	CCreateSaveShm::LoadShm(/*CHART_TP*/int  tp)
{
	...
	/*lame code to debug start*/
	printf("\nStored Procedure performed : %s\n", zQ);
	system("pause");
	/*lame code to debug end*/
	if (FALSE == db->QrySelect(zQ))
	{
		printf( "CHART DATA Load-1 Fail(%s)(%s)", db->GetError(), zQ);
		return FALSE;
	}
	while (db->IsNextRow())
	{
		...
		/*lame code to debug start*/
		printf("\nGroup Key before push_back to listGroupKey: %d\n", listGroupKey);
		/*lame code to debug end*/
	...
	for (it = listGroupKey.begin(); it != listGroupKey.end(); it++)
	{
		...
		/*lame code to debug start*/
		printf("\nStored procedure executed : %s", zQ);
		/*lame code to debug end*/
		...
		while (db->IsNextRow())
		{
			...
			if (!m_shmQ.GroupFind(zGroupKey))
			{
				...
				printf("[GROUP INSERT][%s]", zGroupKey);
				m_shmQ.DataInsert(zGroupKey, (char*)&stShm);
				printf("\n!GroupFind Data: %s\n", stShm);
				//printf("[DATA INSERT][%s][%.*s][%.*s]", zGroupKey, LEN_CHART_NM, stShm.Nm, LEN_CHART_NM, stShm.prevNm);
			...
			else
			{
				if (m_shmQ.DataFind(zGroupKey, zChartNm)<1)
				{
					...
					/*lame code to debug start*/
					//char debug[1024];
					//int ret = m_shmQ.DataFind(zGroupKey, "201710171803");
					//BOOL bRet = m_shmQ.DataGet(zGroupKey, "201710171803",(char*) &debug);
					//printf("\nDATA GET KEY HERE%s\n", debug);
					//system("pause");
					/*lame code to debug end*/
