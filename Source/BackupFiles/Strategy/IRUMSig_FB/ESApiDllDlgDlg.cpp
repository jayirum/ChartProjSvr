...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	// 이미 실행중인지 확인
	if (m_Dup.CheckDuplicate("IRUMStrat_FB") == -1)
	{
		AfxMessageBox("<IRUMStrat_FB.exe>이미 프로세스가 실행중입니다.");
	...
	m_myTray.AddTrayIcon(GetSafeHwnd(), "IRUMStrat_FB");
	...
	// API 경로
	CUtil::GetConfig(g_zConfig, "DIR", "API", szDir);
	m_sDllPath = szDir;
	...
	// API 계정
	CUtil::GetConfig(g_zConfig, "APIINFO", "ID", szDir);
	m_szUserID = szDir;

	CUtil::GetConfig(g_zConfig, "APIINFO", "USER_PWD", szDir);
	m_szUserPass = szDir;

	CUtil::GetConfig(g_zConfig, "APIINFO", "CERT_PWD", szDir);
	m_szCertPass = szDir;

	//CUtil::GetConfig(g_zConfig, "APIINFO", "ACNT_PWD", szDir);
	//m_szHedgePass = szDir;
	
	// 접속서버
	CUtil::GetConfig(g_zConfig, "APIINFO", "REAL_YN", szDir);
	m_cRealYN = szDir[0];
...
{
	m_MDPublish = new CNanoPubSub(TP_PUB);
	return m_MDPublish->Begin(INNER_CHANNEL_PUBSUB);
}

VOID CESApiDllDlgDlg::CESApiDllDlgDlg::ClosePublish()
{
	SAFE_DELETE(m_MDPublish);
}
...
VOID CESApiDllDlgDlg::ResumeThreads()
{
	...
	{
		CAcntThread* p = (CAcntThread*)(*it).second;
		p->ResumeThread();
	}
}
...
LRESULT CESApiDllDlgDlg::OnRecvData(WPARAM wParam, LPARAM lParam)
{
	...
	case AUTO_0985: // 잔고/체결 실시간 
	{
		...
		//if (lpTrOut->ReplyType[0] == '2')
		{
			...
			CAcntThread* pWorker = (CAcntThread*)(*it).second;
...
BOOL CESApiDllDlgDlg::InitMarketDataRecvSocket()
{
	...
	if (!m_MDRecv.Begin(ip, port))
	{
		PrintMsg(LOGTP_FATAL, TRUE, "%s", m_MDRecv.GetMsg());
		return FALSE;
	}
	PrintMsg(LOGTP_SUCC, TRUE, m_MDRecv.GetMsg());
	m_bContinue = TRUE;
	m_MarketDataThread = new Poco::RunnableAdapter<CESApiDllDlgDlg>(*this, &CESApiDllDlgDlg::MarketDataRecvThread);
	m_MDRecvWrapper.start(*m_MarketDataThread);
...
void CESApiDllDlgDlg::CloseMarketDataRecvSocket()
{
	m_MDRecv.End();
	SAFE_DELETE(m_MarketDataThread);
}
...
*/
VOID CESApiDllDlgDlg::MarketDataRecvThread()
{
	PrintMsg(LOGTP_SUCC, FALSE, "MarketDataRecvThread start....");
	while (m_bContinue)
	{
		void* pBuf = m_pMemPool->get();
		int nLen = m_MDRecv.RecvData(pBuf);
		if (nLen < 1)
		{
			PrintMsg(LOGTP_ERR, FALSE, "SISE RECV ERROR:%s", m_MDRecv.GetMsg());
...
void CESApiDllDlgDlg::OnClickedButtonAuto()
{
	...
	CUtil::GetConfig(g_zConfig, "HEDGE_TARGET", "ACNT_HEADER", acnt_header);
	CUtil::GetConfig(g_zConfig, "HEDGE_TARGET", "COUNT", acnt_cnt);

	char tmp[32];
	for (int i = 0; i < atoi(acnt_cnt); i++)
	{
		sprintf(tmp, "%d", i);
		CUtil::GetConfig(g_zConfig, "HEDGE_TARGET", tmp, acnt_body);
...
BOOL CESApiDllDlgDlg::LoadAcntAndUserThread()
{
	...
	while (db->IsNextRow())
	{
		...
		m_mapAcnt[CString(sAcnt)] = new CAcntThread(sAcnt, sPwd);
...
{
	std::map<CString, CAcntThread*>::iterator it;

	for (it = m_mapAcnt.begin(); it != m_mapAcnt.end(); it++)
	{
		CString sAcnt = (*it).first;
		(*it).second = new CAcntThread(sAcnt, &//m_apiObj);
	}
