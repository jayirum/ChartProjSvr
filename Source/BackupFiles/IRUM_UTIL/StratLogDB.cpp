...
#include "ADOFunc.h"
#include "StratLogDB.h"
#include "Util.h"


CStratLogDB::CStratLogDB(_In_ void	*pDBPool)
{
	m_pDBPool = pDBPool;
}


CStratLogDB::~CStratLogDB()
{
	End();
}

VOID CStratLogDB::End()
{}

BOOL CStratLogDB::Begin()
{
	ResumeThread();
	return TRUE;
}

VOID CStratLogDB::Logging(
	...
	ORD_TP ordTp, char cOrdSide, char* psOrdPrc, char* psOrigNo, char* pzMemo)
{
	...
	__except (ReportException(GetExceptionCode(), "CStratLogDB::Logging", m_szMsg))
	{
...
//
//VOID CStratLogDB::SaveCntrLog(_In_ char* pApiPacket)
//{
...
//	}
//	__except (ReportException(GetExceptionCode(), "CStratLogDB::SaveMatchLog", m_szMsg))
//	{
//
//	}
//}
...
VOID CStratLogDB::DBSaveCntr_KR(_In_ char* pApiPacket)
{
	...
	if (!db->ExecQuery(sQ))
	{
		sprintf(m_szMsg, "DB INSERT ERROR(%s)(%s)", sQ, db->GetError());
		return;
	}
}
...
*/
VOID CStratLogDB::DBLoggingWrapper(LOGGING_PARAM_EX* p)
{
	__try
	{
		DBLogging(p);
	}
	__except (ReportException(GetExceptionCode(), "CStratLogDB::DBLogging", m_szMsg))
	{
	}
}

// 
VOID CStratLogDB::DBLogging(LOGGING_PARAM_EX* p)
{
	CDBHandlerAdo db(DBPoolPtr(m_pDBPool)->Get());
	char sQ[1024];
...
#if 0
VOID CStratLogDB::DBLogging_old
(LOGGING_PARAM* p)
{
