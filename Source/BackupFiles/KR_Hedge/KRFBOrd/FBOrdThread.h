...
public:
	...
	BOOL	IsExistAssistPrc() { return (atof(mAssist.zPrc) > 0); }
...
public:
	...
	VOID	Strat_Open(_In_ CString& sSymbol, const CHART_IDX idx, const char* pzGroupKey, const char* pzNowChartNm,
		_In_ ACNT_TRADE_INFO* pTrade, const ST_SHM_CHART_UNIT& nowChart, const ST_SHM_CHART_UNIT& prevChart);
	...
	VOID	Strat_AssistSet(_In_ CString& sSymbol, const CHART_IDX idx, const char* pzGroupKey, const char* pzNowChartNm, 
		_In_ ACNT_TRADE_INFO* pTrade,const ST_SHM_CHART_UNIT& lastChart, const ST_SHM_CHART_UNIT& nowChart);

	// 보조지표 생성 후 청산점검
	VOID	Strat_AssistClr(CString& sSymbol, _In_ ACNT_TRADE_INFO* pTrade, const char* pSise);
