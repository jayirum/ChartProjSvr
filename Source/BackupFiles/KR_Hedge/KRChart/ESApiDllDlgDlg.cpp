...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	// 이미 실행중인지 확인
	if (m_Dup.CheckDuplicate("KRChart") == -1)
	{
		AfxMessageBox("<KRChart> 이미 프로세스가 실행중입니다.");
	...
	{
		PrintMsg(LOGTP_SUCC, TRUE, "%s", m_pServerSock->GetMsg());
		AfxMessageBox("Subscribe 용 소켓 Init 실패");
		return FALSE;
	}
	PrintMsg(LOGTP_SUCC, TRUE, m_pServerSock->GetMsg());
...
{
	std::map<CString, CChartThread*>::iterator it;

	for (it = m_mapSymbol.begin(); it != m_mapSymbol.end(); it++)
	{
		CChartThread* p = (*it).second;
		p->ResumeThread();
	}
}
...
BOOL CESApiDllDlgDlg::LoadSiseSymbol()
{
	...
	{
		PrintMsg(LOGTP_ERR, TRUE, "종목정보 오류(%s)(%s)", sQ.GetBuffer(), db->GetLastError());
		return FALSE;
	}
	...
	while (db->NextRow())
	{
		...
		m_mapSymbol[symbol] = NULL;
		PrintMsg(LOGTP_SUCC, TRUE, "[%s] 차트구성종목", symbol.GetBuffer());
	}
	db->Close();
	...
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
		PrintMsg(LOGTP_ERR, TRUE, "차트종류 오류(%s)(%s)", sQ.GetBuffer(), db->GetLastError());
		return FALSE;
	}

	ZeroMemory(m_chartTp, sizeof(m_chartTp));
	if (db->NextRow())
	{
		if (strncmp("Y", db->Get("CHART_1MIN_YN"), 1) == 0) { m_chartTp[IDX_1MIN] = 'Y';PrintMsg(LOGTP_SUCC, TRUE, "1분차트 구성"); }
		if (strncmp("Y", db->Get("CHART_5MIN_YN"), 1) == 0) { m_chartTp[IDX_5MIN] = 'Y';PrintMsg(LOGTP_SUCC, TRUE, "5분차트 구성"); }
		if (strncmp("Y", db->Get("CHART_10MIN_YN"), 1) == 0) { m_chartTp[IDX_10MIN] = 'Y';PrintMsg(LOGTP_SUCC, TRUE, "10분차트 구성"); }
		if (strncmp("Y", db->Get("CHART_30MIN_YN"), 1) == 0) { m_chartTp[IDX_30MIN] = 'Y';PrintMsg(LOGTP_SUCC, TRUE, "30분차트 구성"); }
		if (strncmp("Y", db->Get("CHART_60MIN_YN"), 1) == 0) { m_chartTp[IDX_60MIN] = 'Y';PrintMsg(LOGTP_SUCC, TRUE, "60분차트 구성"); }
	}
...
BOOL CESApiDllDlgDlg::DBOpen()
{
	...
	{
		PrintMsg(LOGTP_ERR, TRUE, "DB OPEN FAIL(MSG:%s)", m_pDBPool->GetMsg());
		PrintMsg(LOGTP_ERR, TRUE, "(IP:%s)(ID:%s)(PWD:%s)(DB:KRHedgae)", ip, id, pwd);
		SAFE_DELETE(m_pDBPool);
		return FALSE;
	}
...
BOOL CESApiDllDlgDlg::InitThread()
{
	std::map<CString, CChartThread*>::iterator it;
	...
	for (it = m_mapSymbol.begin(); it != m_mapSymbol.end(); it++)
	{
		...
		(*it).second = new CChartThread(symbol, /*m_shm[CHART], m_shm[LASTNM],*/ m_pMemPool, m_pServerSock, m_chartTp);
	}
...
VOID CESApiDllDlgDlg::CloseThread()
{
	std::map<CString, CChartThread*>::iterator it;
	...
	{
		CChartThread* p = (*it).second;
		PostThreadMessage(p->GetMyThreadID(), WM_CHART_ALL_KILL, 0, 0);
		delete p;
	}
	m_mapSymbol.clear();
}
...
void CESApiDllDlgDlg::OnBnClickedOk()
{
	...
	if (!bRet) {
		PrintMsg(LOGTP_ERR, TRUE, m_sDllPath.GetBuffer());
		return;
	}
...
LRESULT CESApiDllDlgDlg::OnServerConnect(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		if(lpstrMsg != NULL){
			...
			PrintMsg(LOGTP_ERR, TRUE, "서버와 연결 실패했습니다(%s)", lpstrMsg);
		}
...
LRESULT CESApiDllDlgDlg::OnRecvData(WPARAM wParam, LPARAM lParam)
{
	...
	if( atoi(szErrCode)!=0 )
	{
		...
		PrintMsg(LOGTP_ERR, TRUE, "ERROR!!! (CODE:%d) (MSG:%.50s)",  pErr->Head.ErrCode, sizeof(pErr->ErrMsg), pErr->ErrMsg);
	}
	...
	case AUTO_0933:// 종목 시세 실시간
	{
		...
		std::map<CString, CChartThread*>::iterator it = m_mapSymbol.find(sShortCode);
		if (it == m_mapSymbol.end())
		{
			PrintMsg(LOGTP_ERR, TRUE, "[%s] 종목은 요청한 종목이 아니다.", sShortCode.GetBuffer());
		}
		else
		{
			...
			pData->seq = ++m_siseSeq;
			CChartThread* p = (*it).second;
			PostThreadMessage(p->GetMyThreadID(), WM_CHART_DATA, 0, (LPARAM)pData);

			//// DB 저장을 위해
			//At0933O_st_Ex* pData2 = (At0933O_st_Ex*)m_pMemPool->get();
			//memcpy(pData2->shortCode, sShortCode.GetBuffer(), LEN_SYMBOL);
			//memcpy(&pData2->org, lpTrOut, sizeof(At0933O_st));
...
BOOL	CESApiDllDlgDlg::SaveAcntInfo(CString &acnt, CString &nm, CString& pwd)
{
	...
	CString sQ;
	sQ.Format("EXEC FB_SAVE_ACNT_INFO '%s', '%s', '%s' ", acnt.GetBuffer(), nm.GetBuffer(), pwd.GetBuffer());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
...
void CESApiDllDlgDlg::OnClose()
{
	...
	m_bContinue = FALSE;
	m_pServerSock->CloseServerSock();
	SAFE_DELETE(m_pServerSock);
	CloseMemPool();
	CloseThread();
	//CloseSHM();
	DBClose();
...
void CESApiDllDlgDlg::OnBnClickedButtonSise()
{
	...
	for (it = m_mapSymbol.begin(); it != m_mapSymbol.end(); ++it)
	{
		...
		PrintMsg(LOGTP_SUCC, TRUE, "%s 시세 요청 (%s)", sSymbol.GetBuffer(), szCode.GetBuffer());
	}
}
	...
	while (p->m_bContinue)
	{
		...
		while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
		{
			...
			case WM_SISE_RECV:
				...
				CDBHandler db(p->m_pDBPool->Get());
				CString sQ;
				sQ.Format("EXEC SYMBOL_CURR_PRC_SAVE '%.*s', '%.*s', '%.*s', '%.*s' ",
					sizeof(pSise->shortCode), pSise->shortCode,
					sizeof(pSise->org.Date), pSise->org.Date,
					sizeof(pSise->org.Time), pSise->org.Time,
					sizeof(pSise->org.ClosePrice), pSise->org.ClosePrice
				);

				if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
				{
					p->PrintMsg(LOGTP_ERR, TRUE, "시세정보 저장 오류(%s)", sQ.GetBuffer());
				}
...
//	if (nRet<1)
//	{
//		PrintMsg(FALSE, TRUE, "[SHM CREATE ERROR](%s)(%s)(%s)", szSHMName, szMutexName, m_shm[CHART].GetErr());
//		return FALSE;
//	}
...
//		)
//	{
//		PrintMsg(FALSE, TRUE, "[SHM SetInitSize ERROR]");
//		return FALSE;
//	}
//
//	// LAST CHART NAME SHM
...
//	if (nRet<1)
//	{
//		PrintMsg(FALSE, TRUE, "[SHM CREATE ERROR](%s)(%s)(%s)", szSHMName, szMutexName, m_shm[LASTNM].GetErr());
//		return FALSE;
//	}
