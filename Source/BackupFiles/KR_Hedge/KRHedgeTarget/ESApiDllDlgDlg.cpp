...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	{
		PrintMsg(FALSE, TRUE, "%s", m_sockSvr.GetMsg());
		AfxMessageBox("Subscribe 용 소켓 Init 실패");
		return FALSE;
	}
	PrintMsg(TRUE, TRUE, "Server Sock Init Ok(%s)", m_sockSvr.GetMsg());
...
//#if 0	// DB 에서 읽지 않고, 로그인해서 내려오는 모든 계좌가 대상이 된다.
	// 헷지대상 계좌 load
...
void CESApiDllDlgDlg::OnBnClickedOk()
{
	...
	PrintMsg(TRUE, TRUE, "API LOGIN 시도(ID:%s)(접속비번:%s)(인증비번:%s)(RealServerYN:%c)",
...
LRESULT CESApiDllDlgDlg::OnRecvData(WPARAM wParam, LPARAM lParam)
{
	...
	if( atoi(szErrCode)!=0 )
	{
		...
		PrintMsg(FALSE, TRUE, "ERROR!!! (CODE:%.4s) (MSG:%.50s)",  pErr->Head.ErrCode,  pErr->ErrMsg);
	}
	...
	case AUTO_0985: // // 잔고/체결 실시간 
	{
		...
		*/
		if (lpTrOut->ReplyType[0] == '2') 
		{
			...
			m_sockSvr.SendData((void*)lParam, sizeof(At0985O_st));
...
BOOL	CESApiDllDlgDlg::SaveAcntInfo(CString &acnt, CString &nm)
{
	//if (!CheckDBPool())
	//{
	//	PrintMsg(FALSE, TRUE, "DB OPEN 오류");
...
void CESApiDllDlgDlg::OnClickedButtonAuto()
{
	...
	if (nCnt > 0) {
		...
		for (int i = 0; i < nCnt; i++)
		{
			...
			memcpy(lpAuto->AutoKey, pAcnt[i], pAcnt[i].GetLength());
			m_Api.SetAutoUpdate(TRUE, TRUE, szBuff);
			PrintMsg(TRUE, TRUE, "[%s] 계좌 자동업데이트 설정", pAcnt[i].GetBuffer());
		}
		delete[] pAcnt;
	}
