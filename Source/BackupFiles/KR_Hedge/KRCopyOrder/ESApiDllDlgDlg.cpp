...
BOOL CESApiDllDlgDlg::OnInitDialog()
{
	...
	// 이미 실행중인지 확인
	if (m_Dup.CheckDuplicate("KRCopyOrder") == -1)
	{
		AfxMessageBox("<KRCopyOrder>이미 프로세스가 실행중입니다.");
	...
	m_sDllPath = szDir;
	IR_PrintMsg(TRUE, TRUE, "API 저장된 폴더 : %s", m_sDllPath.GetBuffer());

	// 메모리 pool
	if(!IR_InitMemPool()) {
		AfxMessageBox("memory pool Init 실패");
		return FALSE;
	}
...
{
	return IR_OnServerConnect(wParam, lParam);
}

LRESULT CESApiDllDlgDlg::IR_OnServerConnect(WPARAM wParam, LPARAM lParam)
{
...
VOID CESApiDllDlgDlg::IR_TargetRecvThread()
{
	...
	std::map<CString, CCopyOrder*>::iterator it;
	while (m_bContinue)
	{
		void* pBuf = m_pMemPool->get();
		ZeroMemory(pBuf, MEMPOOL_BLOCK_SIZE);
		int nLen = m_targetRcv.RecvData(MEMPOOL_BLOCK_SIZE, pBuf);
		if (nLen < 1)
		{
		...
		IR_PrintMsg(TRUE, FALSE, "RECV(%s)", pBuf);

		// 모든 Thread 에게 Posting
		for (it = m_mapAcnt.begin(); it != m_mapAcnt.end(); it++) {
			PostThreadMessage(((*it).second)->ThrId(), WM_COPYTHREAD_RECV, 0, (LPARAM)pBuf);
		}
...
{
	return IR_OnAcctList(wParam, lParam);
}
	
LRESULT	CESApiDllDlgDlg::IR_OnAcctList(WPARAM wParam, LPARAM lParam)
{
...
BOOL	CESApiDllDlgDlg::IR_SaveAcntInfo(CString &acnt, CString &nm)
{
	...
	sQ.Format("EXEC SAVE_ACNT_INFO '%s','%s','%s', '%s' ", IR_BROKER_KR, m_szUserID, acnt.GetBuffer(), nm.GetBuffer());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
...
void CESApiDllDlgDlg::OnClose()
{
	...
	std::map<CString, CCopyOrder*>::iterator it;
	for (it = m_mapAcnt.begin(); it != m_mapAcnt.end(); it++) {
		PostThreadMessage(((*it).second)->ThrId(), WM_COPYTHREAD_DIE, 0, (LPARAM)0);
		delete (*it).second;
	}
	m_mapAcnt.clear();
	IR_CloseMemPool();
...
{
	IR_OnClickedButtonAuto();
}

void CESApiDllDlgDlg::IR_OnClickedButtonAuto()
{
	...
	std::map<CString, CCopyOrder*>::iterator it;
	for (it = m_mapAcnt.begin(); it != m_mapAcnt.end(); it++) {
		CString sAcnt = ((*it).second)->Acnt();
		memcpy(lpAuto->AutoKey, sAcnt, sAcnt.GetLength());
		m_Api.SetAutoUpdate(TRUE, TRUE, szBuff);
		IR_PrintMsg(TRUE, TRUE, "[%s] 계좌 자동업데이트 설정", sAcnt.GetBuffer());
	}
...
BOOL	CESApiDllDlgDlg::IR_LoadCopyOrderAcnt()
{
	CString sQ;
	CDBHandler db(m_pDBPool->Get());

	sQ.Format("SELECT ACNT_NO, ACNT_PWD, QTY_MULTIPLE FROM ACNT_INFO WHERE BROKER_ID='%s' AND MOTHER_ID='%s' AND COPY_ORD_YN='Y' ", 
				IR_BROKER_KR, m_szUserID.GetBuffer());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
	...
	while (db->NextRow())
	{
		CString sAcntNo = db->Get("ACNT_NO");
		UINT unMutiple = db->GetInt("QTY_MULTIPLE");
		if (unMutiple == 0)	unMutiple = 1;

		CCopyOrder* p = new CCopyOrder(sAcntNo.GetBuffer(), db->Get("ACNT_PWD"), &m_Api, &m_csAPI,
			m_pMemPool, m_pDBPool, m_hWnd, unMutiple);
		p->ResumeThread();
		m_mapAcnt[sAcntNo] = p;
	}
	return TRUE;
...
#if 0
// 헷지주문계좌정보 조회
BOOL	CESApiDllDlgDlg::IR_LoadCopyOrderAcnt_Old()
{
	char tmp[128];
