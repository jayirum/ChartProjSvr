...
#define ID_TIME_REG_CNTR_1		102	// 체결자동업데이트 등록
#define ID_TIME_REG_CNTR_2		103	// 체결자동업데이트 등록
#define ID_TIME_REG_CNTR_3		104	// 체결자동업데이트 등록
...
BOOL CHDFCommClientDlg::OnInitDialog()
{
	...
	// 이미 실행중인지 확인
	if (m_Dup.CheckDuplicate("HDCopyTarget") == -1)
	{
		AfxMessageBox("<HDCopyTarget>이미 프로세스가 실행중입니다.");
	...
	// 헷지대상 계좌 load
	//if (!IR_LoadHedgeTarget(TRUE/*RESET*/)) {
	...
	if (!m_sockSvr.InitServerSock(ip, port))
	{
		IR_PrintMsg(LOGTP_FATAL, TRUE, "%s", m_sockSvr.GetMsg());
		AfxMessageBox("Subscribe 용 소켓 Init 실패");
		return FALSE;
	}
	IR_PrintMsg(LOGTP_SUCC, TRUE, "Server Sock Init Ok(%s)", m_sockSvr.GetMsg());

	// noted thread
	unsigned int dwThreadID = 0;
	_beginthreadex(NULL, 0, &IR_NotedThreadWrapper, this, 0, &dwThreadID);
	m_bContinue = TRUE;
...
BOOL	CHDFCommClientDlg::IR_SaveAcntInfo(CString &acnt, CString &nm)
{
	//if (!CheckDBPool())
	//{
	...
	sQ.Format("EXEC SAVE_ACNT_INFO '%s', '%s', '%s', '%s' ", IR_BROKER_HD, IR_ID(), acnt.GetBuffer(), nm.GetBuffer());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
		IR_PrintMsg(LOGTP_ERR, TRUE, "계좌정보 저장 오류z(%s)", sQ.GetBuffer());
		return FALSE;
	}
...
void CHDFCommClientDlg::OnGetBroadData(CString strKey, LONG nRealType)
{
	...
	switch (nRealType)
	{
		...
		case 189: //해외체결내역
		{
			CString strAcntNo = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "계좌번호");
			...
			CString strOrdP = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "체결가격");
			CString strOrdQ = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "체결수량");
			CString strOrdTp = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "주문유형"); // 1:시장가, 2:지정가, 3:STOP, 4:STOP지정가
			CString strCmsn = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "수수료");
			CString strDT = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "체결일자");
			CString strTM = m_CommAgent.CommGetData(strKey, nRealType, "OutRec1", 0, "체결시간");
			...
			// 헷지대상이 아니면 skip
			if (!m_hedgeTarget.IsTarget(strAcntNo)) {
				IR_PrintMsg(LOGTP_SUCC, FALSE, "체결수신.헷지대상아니어서 SKIP(수신계좌:%s)", strAcntNo.GetBuffer());
				break;
			}


			// 헷지처리를 위해 KRHedgeOrder 로 전달. ==> SOCKET
			char zSendbuf[1024] = { 0, };
			COPYORD_TARGET_CNTR* p = (COPYORD_TARGET_CNTR*)zSendbuf;
			
			p->TradeTp[0] = DEF_TRADEGB_COPY;
			sprintf(p->Broker, IR_BROKER_HD);
			sprintf(p->symbol, "%*.*s", sizeof(p->symbol), sizeof(p->symbol), strSeries.GetBuffer());
			sprintf(p->side, "%*.*s", sizeof(p->side), sizeof(p->side), strBySlTp.GetBuffer());
			sprintf(p->prc, "%*.*s", sizeof(p->prc), sizeof(p->prc), strOrdP.GetBuffer());
			sprintf(p->qty, "%*.*s", sizeof(p->qty), sizeof(p->qty), strOrdQ.GetBuffer());
			memset(p->multiple, 0x20, sizeof(p->multiple));
			sprintf(p->TargetAcntNo, "%-*.*s", sizeof(p->TargetAcntNo), sizeof(p->TargetAcntNo), strAcntNo.GetBuffer());
			sprintf(p->TargetOrdNo, "%-*.*s", sizeof(p->TargetOrdNo), sizeof(p->TargetOrdNo), strOrdNo.GetBuffer());
			if(strOrdTp.Compare("1")==0)
				sprintf(p->TargetOrdTp, "%-*.*s", sizeof(p->TargetOrdTp), sizeof(p->TargetOrdTp), "시장가");
			if (strOrdTp.Compare("2") == 0)
				sprintf(p->TargetOrdTp, "%-*.*s", sizeof(p->TargetOrdTp), sizeof(p->TargetOrdTp), "지정가");
			if (strOrdTp.Compare("3") == 0)
				sprintf(p->TargetOrdTp, "%-*.*s", sizeof(p->TargetOrdTp), sizeof(p->TargetOrdTp), "STOP");
			if (strOrdTp.Compare("4") == 0)
				sprintf(p->TargetOrdTp, "%-*.*s", sizeof(p->TargetOrdTp), sizeof(p->TargetOrdTp), "STOP지정가");

			sprintf(p->TargetCmsn, "%*.*s", sizeof(p->TargetCmsn), sizeof(p->TargetCmsn), strCmsn.GetBuffer());
			sprintf(p->TargetDT, "%*.*s", sizeof(p->TargetDT), sizeof(p->TargetDT), strDT.GetBuffer());
			sprintf(p->TargetTM, "%*.*s", sizeof(p->TargetTM), sizeof(p->TargetTM), strTM.GetBuffer());


			m_sockSvr.SendData((void*)zSendbuf, sizeof(COPYORD_TARGET_CNTR));

			CString s; 
			s.Format("<체결실시간>[ACC:%s] [종목코드:%s] [주문번호:%s] [매매구분:%s][체결가격:%s][체결수량:%s](PACK:%s)",
				strAcntNo.GetBuffer(), strSeries.GetBuffer(), strOrdNo.GetBuffer(), strBySlTp.GetBuffer(), strOrdP.GetBuffer(),
				strOrdQ.GetBuffer(), zSendbuf);

			IR_PrintMsg(LOGTP_SUCC, FALSE, s.GetBuffer());
			...
			//	IR_PrintMsg(LOGTP_SUCC, TRUE, strMsg.GetBuffer());
				//	[I][20:30 : 03.980]체결 주문번호[17NC001979]종목[]매매[1]가격[46.25]수량[2]
			//}
		}
		break;
		//case 196: //해외주문접수
...
long CHDFCommClientDlg::OnTrayIcon(WPARAM wParam, LPARAM IParam)
{
...
void CHDFCommClientDlg::OnAppExit(void)
{
	m_myTray.DelTrayIcon(GetSafeHwnd());
	CDialog::OnCancel();
}

void CHDFCommClientDlg::OnDialogShow(void)
{
...
BOOL CHDFCommClientDlg::IR_SaveTradeInfo(CString& sShortCode, char* pTarget, char* pHedge)
{
	// Get DB Session
	//if (!CheckDBPool())
	//{
	...
	//	return FALSE;
	//}
	//IR_PrintMsg(TRUE, TRUE, "%s[DB저장](ACC:%.*s)(종목:%.*s)(방향:%.*s)(수량:%.*s)",
...
// 헷지대상계좌 조회
BOOL CHDFCommClientDlg::IR_LoadHedgeTarget(BOOL bReset)
{
	CString sQ;
	CDBHandler db(m_pDBPool->Get());

	sQ.Format("SELECT ACNT_NO FROM ACNT_INFO WHERE BROKER_ID='%s' AND MOTHER_ID='%s' AND COPY_TARGET_YN='Y'", IR_BROKER_HD, IR_ID());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
		IR_PrintMsg(LOGTP_ERR, TRUE, "헷지대상 계좌정보 조회 오류(%s)(%s)", sQ.GetBuffer(), db->GetLastError());
		return FALSE;
	}
...
BOOL CHDFCommClientDlg::IR_DBOpen()
{
	...
	{
		IR_PrintMsg(LOGTP_ERR, TRUE, "DB OPEN FAIL(MSG:%s)", m_pDBPool->GetMsg());
		IR_PrintMsg(LOGTP_ERR, TRUE, "(IP:%s)(ID:%s)(PWD:%s)(DB:KRHedgae)", ip, id, pwd);
...
void CHDFCommClientDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	switch (nIDEvent)
	{
		...
		break;
		case ID_TIME_REG_CNTR_1:
			KillTimer(ID_TIME_REG_CNTR_1);
			IR_LoadHedgeTarget(TRUE);
			break;
		case ID_TIME_REG_CNTR_2:
			KillTimer(ID_TIME_REG_CNTR_2);
			IR_LoadHedgeTarget(TRUE);
			break;
		case ID_TIME_REG_CNTR_3:
			KillTimer(ID_TIME_REG_CNTR_3);
			IR_LoadHedgeTarget(TRUE);
			break;
	}
...
unsigned WINAPI CHDFCommClientDlg::IR_NotedThreadWrapper(LPVOID lp)
{
	CHDFCommClientDlg* p = (CHDFCommClientDlg*)lp;
	p->IR_NotedThread();
	return 0;
}

VOID CHDFCommClientDlg::IR_NotedThread()
{
	CString sQ;
	while (m_bContinue)
	{
