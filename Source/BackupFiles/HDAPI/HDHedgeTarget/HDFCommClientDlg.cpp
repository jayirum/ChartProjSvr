...
BOOL	CHDFCommClientDlg::IR_SaveAcntInfo(CString &acnt, CString &nm)
{
	...
	sQ.Format("EXEC SAVE_ACNT_INFO '%s', '%s', '%s', '%s' ", BROKER_HD, IR_ID(), acnt.GetBuffer(), nm.GetBuffer());
...
void CHDFCommClientDlg::OnGetBroadData(CString strKey, LONG nRealType)
{
	switch (nRealType)
	{
		...
		case 189: //해외체결내역
		{
			...
			HEDGE_TARGET_CNTR* p = (HEDGE_TARGET_CNTR*)zSendbuf;
			...
			m_sockSvr.SendData((void*)zSendbuf, sizeof(HEDGE_TARGET_CNTR));
...
BOOL CHDFCommClientDlg::IR_LoadHedgeTarget(BOOL bReset)
{
	...
	sQ.Format("SELECT ACNT_NO FROM ACNT_INFO WHERE BROKER_ID='%s' AND MOTHER_ID='%s' AND ACNT_TP=1", BROKER_HD, IR_ID());
	if (!db->Cmd(sQ.GetBuffer()) || !db->Exec())
	{
