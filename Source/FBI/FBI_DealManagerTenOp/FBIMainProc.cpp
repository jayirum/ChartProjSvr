...
{
	//m_unOrdRecv = m_unSaveData = 0;
	//m_hOrdRecv = m_hSaveData = NULL;
	m_pDBPool = NULL;
	m_bContinue = TRUE;
	//m_pApiClient = NULL;
...
BOOL CFBIMainProc::Initialize()
{
	...
	// create recv thread
	m_hOrdRecv = (HANDLE)_beginthreadex(NULL, 0, &Thread_OrdRecv, this, 0, &m_unOrdRecv);
...
{
	PostThreadMessage(m_unOrdRecv, _FBI::WM_TERMINATE, 0, 0);
	SAFE_CLOSEHANDLE(m_hOrdRecv);
...
BOOL CFBIMainProc::LoadStkCode()
{
	...
	else
	{
		while (db->IsNextRow())
		{
			...
			CDBHandlerAdo db2(m_pDBPool->Get());
			sprintf(zQ, "EXEC AA_LOG_PROCESS_STATUS 'DealManagerTenOp', '[%s] 초기화 성공.' ", zStCd);
			if (FALSE == db2->ExecQuery(zQ))
			{
				g_log.log(ERR/*NOTIFY*/, "AA_LOG_PROCESS_STATUS Error(%s)", zQ);
			}
...
/*
	Recv Order
	PostMessage WM_ORD_RECV
*/
unsigned WINAPI CFBIMainProc::Thread_OrdRecv(LPVOID lp)
{
