...
VOID CStkOrdManager::OrderProc(void* pOrdData)
{
	...
	//char zName[32];
	//g_log.log(INFO, "[%s][RECV](SYMBOL:%s)(OrdStatus:%d)(U/D:%c)(OrdPrc:%.5f)(WinPrc:%.5f)(LosePrc:%.5f)",
...
VOID CStkOrdManager::OrderProcAdd(_FBI::ST_SLORD* pIn, std::string sAscPrc, std::string sDescPrc)
{
	...
	g_log.log(INFO, "[ADD-ASC ](OrdNo:%d)(OrdPrc:%f)(AscPrc:%s)(UpDn:%c)(MapCnt:%d)(ListCnt:%d)",
		pIn->OrdNo, pIn->dOrdPrc, sAscPrc.c_str(), pIn->cUpDn, m_mapAsc.size(), lstOrdAsc.size());
	...
	g_log.log(INFO, "[ADD-DESC](OrdNo:%d)(OrdPrc:%f)(DescPrc:%s)(UpDn:%c)(MapCnt:%d)(ListCnt:%d)",
		pIn->OrdNo, pIn->dOrdPrc, sDescPrc.c_str(), pIn->cUpDn, m_mapDesc.size(), lstOrdAsc.size());
}
...
BOOL CStkOrdManager::DeleteOrder(EN_DIRECTION enDirection, std::string sOrdPrc, int nOrdNo)
{
	...
	if (enDirection==EN_ASC)
	{
		...
		if (itAsc == m_mapAsc.end())
		{
			g_log.log(INFO, "[DEL-ASC ](OrdNo:%d)(%s)(%s)ASC Map 에 없음.", 
				nOrdNo, sOrdPrc.c_str(), GetThreadName(zName));
			return FALSE;
		}

		lstOrd = (*itAsc).second;
		if (lstOrd.size()==0) {
			g_log.log(INFO, "[DEL-ASC ](OrdNo:%d)(%s)List 가 비어 있음.", 
						nOrdNo, GetThreadName(zName));
		}
		...
		if(bErased)
			g_log.log(INFO, "[DEL-ASC ](OrdNo:%d)(Prc:%s)(MapCnt:%d)(ListCnt:%d)", 
				nOrdNo, sOrdPrc.c_str(), m_mapAsc.size(), lstOrd.size());
	}
	...
	if(enDirection == EN_DESC)
	{
		...
		if (itDesc == m_mapDesc.end())
		{
			g_log.log(INFO, "[DEL-DESC](OrdNo:%d)(%s)(%s)DESC Map 에 없음.", nOrdNo, sOrdPrc.c_str(), GetThreadName(zName));//LOG
			return FALSE;
		}

		lstOrd = (*itDesc).second; 
		if (lstOrd.size()==0) {
			g_log.log(INFO, "[DEL- DESC](OrdNo:%d)(%s)List 가 비어 있음.", nOrdNo, GetThreadName(zName));
		}
		for (itList = lstOrd.begin(); itList != lstOrd.end(); )
		{
			if (nOrdNo == (*itList)->OrdNo)
		...
		if(bErased)
			g_log.log(INFO, "[DEL-DESC](OrdNo:%d)(Prc:%s)(MapCnt:%d)(ListCnt:%d)",
				nOrdNo, sOrdPrc.c_str(), m_mapAsc.size(), lstOrd.size());
	}
	return bErased;
}
...
VOID CStkOrdManager::ScanOrdByPrc(void* pPrcData)
{
	std::string				sOrdPrc;
	std::string				sCurrPrc;
	_FBI::SMPACK_FX_EXEC*	pNowPrc;
	double					dCurrPrc;
	char					temp	[32] = { 0, };
	char					zName	[32] = { 0, };

	pNowPrc		= (_FBI::SMPACK_FX_EXEC*)pPrcData; 
	dCurrPrc	= atof(pNowPrc->cup);

	if (dCurrPrc <= 0 )
	{
		g_log.log(INFO, "[ScanOrdByPrc]Price is Zero(%s)(%s)(%s)"
			, GetThreadName(zName), pNowPrc->issue, sCurrPrc.c_str());
		return;
	}
