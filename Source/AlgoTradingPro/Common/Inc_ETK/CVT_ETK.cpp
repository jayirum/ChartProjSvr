...
int CCVT_ETK::de_FU_SiseEx(char* psOrgData
...
)
{
	...
	OVC_OutBlock*	 org = (OVC_OutBlock*)psOrgData;
	DF_CME_TICK_OUT* p = (DF_CME_TICK_OUT*)pOut;

	memset(p, 0x20, sizeof(DF_CME_TICK_OUT));

	SET_LEN(sizeof(DF_CME_TICK_OUT), szLen);

	p->STX[0] = 0x02;
	memcpy(p->Header.Len, szLen, sizeof(p->Header.Len));
	memcpy(p->Header.Code, CD_PACKET_DATAFEED_CME_TICK_OUT, sizeof(p->Header.Code));
	getGMTtime(t);
	memcpy(p->Header.GMTTime, t, sizeof(p->Header.GMTTime));

	sprintf(t, "%-*.*s", SIZE_SYMBOL, SIZE_SYMBOL, org->symbol);
	change_etk_to_normal(t, szStkCode);

	memcpy(p->Symbol, szStkCode, min(strlen(szStkCode), sizeof(org->symbol)));
	memcpy(p->TradeTM, org->trdtm, min(sizeof(p->TradeTM), sizeof(org->trdtm)));

	get_double_withdot2(szStkCode, org->curpr, sizeof(org->curpr), SIZE_PRC, t);
	memcpy(p->Tick, t, sizeof(p->Tick));
	...
	//1-018-009-007-004.
	if (org->cgubun[0] == '-')
		sprintf(t, "%*d", sizeof(p->Vol), -1 * S2N(org->trdq, sizeof(org->trdq)));
	else
		sprintf(t, "%*d", sizeof(p->Vol), S2N(org->trdq, sizeof(org->trdq)));
	memcpy(p->Vol, t, sizeof(p->Vol));
...
int CCVT_ETK::de_FU_Hoga(char* psOrgData, char* pOut)
{
	...
	DF_CME_QUOTE_OUT*	p = (DF_CME_QUOTE_OUT*)pOut;			//	PacketDef.h
	...
	*(pOut + sizeof(DF_CME_QUOTE_OUT)) = 0x00;
	...
	//	종목코드	
	sprintf(t, "%-*.*s", sizeof(p->Symbol), sizeof(p->Symbol), org->symbol);
	change_etk_to_normal(t, szStkCode);

	memcpy(p->Symbol, szStkCode, min(strlen(szStkCode), sizeof(org->symbol)));

	//	시각
	memcpy(p->TradeTM, org->hotime, min(sizeof(p->TradeTM), sizeof(org->hotime)));

	//-------------------------------------------------------------------------------------------------//
	//	매도호가 1
	get_double_withdot2(szStkCode, org->offerho1, sizeof(org->offerho1), sizeof(p->ask.p1), t);
	memcpy(p->ask.p1, t, sizeof(p->ask.p1));

	//	매도호가 2
	get_double_withdot2(szStkCode, org->offerho2, sizeof(org->offerho2), sizeof(p->ask.p2), t);
	memcpy(p->ask.p2, t, sizeof(p->ask.p2));

	//	매도호가 3
	get_double_withdot2(szStkCode, org->offerho3, sizeof(org->offerho3), sizeof(p->ask.p3), t);
	memcpy(p->ask.p3, t, sizeof(p->ask.p3));

	//	매도호가 4
	get_double_withdot2(szStkCode, org->offerho4, sizeof(org->offerho4), sizeof(p->ask.p4), t);
	memcpy(p->ask.p4, t, sizeof(p->ask.p4));

	//	매도호가 5
	get_double_withdot2(szStkCode, org->offerho5, sizeof(org->offerho5), sizeof(p->ask.p5), t);
	memcpy(p->ask.p5, t, sizeof(p->ask.p5));

	//-------------------------------------------------------------------------------------------------//
	sprintf(t, "%*d", sizeof(p->ask.v1), S2N(org->offerrem1, sizeof(org->offerrem1)));
	memcpy(p->ask.v1, t, sizeof(p->ask.v1));

	sprintf(t, "%*d", sizeof(p->ask.v2), S2N(org->offerrem2, sizeof(org->offerrem2)));
	memcpy(p->ask.v2, t, sizeof(p->ask.v2));

	sprintf(t, "%*d", sizeof(p->ask.v3), S2N(org->offerrem3, sizeof(org->offerrem3)));
	memcpy(p->ask.v3, t, sizeof(p->ask.v3));

	sprintf(t, "%*d", sizeof(p->ask.v4), S2N(org->offerrem4, sizeof(org->offerrem4)));
	memcpy(p->ask.v4, t, sizeof(p->ask.v4));

	sprintf(t, "%*d", sizeof(p->ask.v5), S2N(org->offerrem5, sizeof(org->offerrem5)));
	memcpy(p->ask.v5, t, sizeof(p->ask.v5));

	sprintf(t, "%*d", sizeof(p->ask.v_sum), S2N(org->totofferrem, sizeof(org->totofferrem)));
	memcpy(p->ask.v_sum, t, sizeof(p->ask.v_sum));

	//-------------------------------------------------------------------------------------------------//
	sprintf(t, "%*d", sizeof(p->ask.c1), S2N(org->offerno1, sizeof(org->offerno1)));
	memcpy(p->ask.c1, t, sizeof(p->ask.c1));

	sprintf(t, "%*d", sizeof(p->ask.c2), S2N(org->offerno2, sizeof(org->offerno2)));
	memcpy(p->ask.c2, t, sizeof(p->ask.c2));

	sprintf(t, "%*d", sizeof(p->ask.c3), S2N(org->offerno3, sizeof(org->offerno3)));
	memcpy(p->ask.c3, t, sizeof(p->ask.c3));

	sprintf(t, "%*d", sizeof(p->ask.c4), S2N(org->offerno4, sizeof(org->offerno4)));
	memcpy(p->ask.c4, t, sizeof(p->ask.c4));

	sprintf(t, "%*d", sizeof(p->ask.c5), S2N(org->offerno5, sizeof(org->offerno5)));
	memcpy(p->ask.c5, t, sizeof(p->ask.c5));

	sprintf(t, "%*d", sizeof(p->ask.c_sum), S2N(org->totoffercnt, sizeof(org->totoffercnt)));
	memcpy(p->ask.c_sum, t, sizeof(p->ask.c_sum));

	/////////////////////////////////////////////////////////////////////////////////////////////////////////

	//	매수호가 1
	get_double_withdot2(szStkCode, org->bidho1, sizeof(org->bidho1), sizeof(p->bid.p1), t);
	memcpy(p->bid.p1, t, sizeof(p->bid.p1));

	//	매수호가 2
	get_double_withdot2(szStkCode, org->bidho2, sizeof(org->bidho2), sizeof(p->bid.p2), t);
	memcpy(p->bid.p2, t, sizeof(p->bid.p2));

	//	매수호가 3
	get_double_withdot2(szStkCode, org->bidho3, sizeof(org->bidho3), sizeof(p->bid.p3), t);
	memcpy(p->bid.p3, t, sizeof(p->bid.p3));

	//	매수호가 4
	get_double_withdot2(szStkCode, org->bidho4, sizeof(org->bidho4), sizeof(p->bid.p4), t);
	memcpy(p->bid.p4, t, sizeof(p->bid.p4));

	//	매수호가 5
	get_double_withdot2(szStkCode, org->bidho5, sizeof(org->bidho5), sizeof(p->bid.p5), t);
	memcpy(p->bid.p5, t, sizeof(p->bid.p5));

	//-------------------------------------------------------------------------------------------------//
	sprintf(t, "%*d", sizeof(p->bid.v1), S2N(org->bidrem1, sizeof(org->bidrem1)));
	memcpy(p->bid.v1, t, sizeof(p->bid.v1));

	sprintf(t, "%*d", sizeof(p->bid.v2), S2N(org->bidrem2, sizeof(org->bidrem2)));
	memcpy(p->bid.v2, t, sizeof(p->bid.v2));

	sprintf(t, "%*d", sizeof(p->bid.v3), S2N(org->bidrem3, sizeof(org->bidrem3)));
	memcpy(p->bid.v3, t, sizeof(p->bid.v3));

	sprintf(t, "%*d", sizeof(p->bid.v4), S2N(org->bidrem4, sizeof(org->bidrem4)));
	memcpy(p->bid.v4, t, sizeof(p->bid.v4));

	sprintf(t, "%*d", sizeof(p->bid.v5), S2N(org->bidrem5, sizeof(org->bidrem5)));
	memcpy(p->bid.v5, t, sizeof(p->bid.v5));

	sprintf(t, "%*d", sizeof(p->bid.v_sum), S2N(org->totbidrem, sizeof(org->totbidrem)));
	memcpy(p->bid.v_sum, t, sizeof(p->bid.v_sum));

	//-------------------------------------------------------------------------------------------------//

	sprintf(t, "%*d", sizeof(p->bid.c1), S2N(org->bidno1, sizeof(org->bidno1)));
	memcpy(p->bid.c1, t, sizeof(p->bid.c1));

	sprintf(t, "%*d", sizeof(p->bid.c2), S2N(org->bidno2, sizeof(org->bidno2)));
	memcpy(p->bid.c2, t, sizeof(p->bid.c2));

	sprintf(t, "%*d", sizeof(p->bid.c3), S2N(org->bidno3, sizeof(org->bidno3)));
	memcpy(p->bid.c3, t, sizeof(p->bid.c3));

	sprintf(t, "%*d", sizeof(p->bid.c4), S2N(org->bidno4, sizeof(org->bidno4)));
	memcpy(p->bid.c4, t, sizeof(p->bid.c4));

	sprintf(t, "%*d", sizeof(p->bid.c5), S2N(org->bidno5, sizeof(org->bidno5)));
	memcpy(p->bid.c5, t, sizeof(p->bid.c5));

	sprintf(t, "%*d", sizeof(p->bid.c_sum), S2N(org->totbidcnt, sizeof(org->totbidcnt)));
	memcpy(p->bid.c_sum, t, sizeof(p->bid.c_sum));

	p->ETX[0] = 0x03;
	p->ETX[1] = 0x00;

	return sizeof(DF_CME_QUOTE_OUT);
...
void CCVT_ETK::ETK_SetPacketData( char* psData, int nSize, LPCTSTR pszSrc, int nType, int nDotPos )
{
	...
	else if( nType == DATA_TYPE_FLOAT )
	{
		...
		//todo ASSERT( nDotPos >= nSrcDotLen );
...
char* CCVT_ETK::get_double_withdot2( char* i_psStkCode, char* i_psOrg, int i_nOrgLen, int i_nOutLen, char* o_psOut )
{
	char	t[128], t2[128];
	double	dTemp;
	int nDotCnt = getDotCnt(i_psStkCode);
