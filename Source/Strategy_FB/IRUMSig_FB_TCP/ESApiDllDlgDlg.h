...
#include "../../IRUM_INC/KR/ESApiMessage.h"
...
#define APP_VERSION	"20170828_Ver.1"
//
...
protected:
	...
	Poco::RunnableAdapter<CESApiDllDlgDlg> *m_MarketDataThread;	// 시세수신 스레드를 위해
	CNanoPull				m_MDRecv;
	Poco::Thread			m_MDRecvWrapper;
	
	Poco::RunnableAdapter<CESApiDllDlgDlg> *m_notedThread;	// 관리자로 부터 통보 받는 스레드를 위해
	Poco::Thread			m_notedWrapper;

	CNanoPubSub				*m_MDPublish;
	//todo. 이동. CStratLog				*m_pStratLog;	// 전략수행 로깅 - DB
	CTcpSrv					*m_pNoteSock;
	CBlockDup				m_Dup;
// 구현입니다.
protected:
...
public:
	...
	// ChartApiKR.exe 에서 시세를 받는다.
	BOOL	InitMarketDataRecvSocket();
	VOID	CloseMarketDataRecvSocket();
	VOID	MarketDataRecvThread();
	...
	//TODO. BOOL	InitStratLog();
	//TODO. VOID	CloseStratLog();
