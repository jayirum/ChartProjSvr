...
VOID CSignalOpenPrc::CheckEmptyArrow()
{
	...
	*/
	...
	ST_SHM_CHART_UNIT	chartMatched;
	...
	for ( i= 0; i < nLoop; i++)
	{
		...
		g_log.log(LOGTP_SUCC, "<EMPTY:%d><%s>[NOW:%.12s](%c)(%10.5f)",
			i, m_p.zGroupKey, chartUnit[i].Nm, chartUnit[i].gb[0], atof(chartUnit[i].open));
		...
		memcpy(&chartMatched, &chartUnit[i], sizeof(chartMatched));
		// 양봉 또는 음봉인가?
	...
	if (nFoundPlus > -1)
	{
		...
		pResult->ChartGb[0] = CD_OPENSIG_PLUS;
		SetLastCheckEmptyArrow();

		//// 매수Signal 표시 (0606)
		//if ((nowChart.signalGb[0] != DEF_SELL) && (nowChart.signalGb[0] != DEF_BUY)) {
		//	SetSignal(DEF_BUY, pzGroupKey, pzNowChartNm);
		sprintf(m_zMsg, "[OPEN_EMPTY_PLUS][%s]"
						"[NOWCHART:%.12s][NOWCHART_OPEN:%.20s]"
						"[MATCHEDCHART:%.12s][MATCHED_OPEN:%.20s]"
						"[SIG GB:%c]",
						m_p.zGroupKey, 
						m_p.pChartNow->Nm, m_p.pChartNow->open,
						chartMatched.Nm, chartMatched.open,
						pResult->ChartGb[0]
						);
	...
	if (nFoundMinus > -1)
	{
		...
		CROSS_TP crossTp = GetCross(m_p.pChartNow);
		pResult->nCrossTp = (int)crossTp;
		pResult->ChartGb[0] = CD_OPENSIG_MINUS;
		SetLastCheckEmptyArrow();

		sprintf(m_zMsg, "[OPEN_EMPTY_MINUS][%s]"
			"[NOWCHART:%.12s][NOWCHART_OPEN:%.20s]"
			"[MATCHEDCHART:%.12s][MATCHED_OPEN:%.20s]"
			"[SIG GB:%c]",
			m_p.zGroupKey,
...
VOID CSignalOpenPrc::CheckFilledArrow()
{
	...
	strcpy(pResult->zGroupKey, m_p.zGroupKey);
	//sprintf(pResult->zChartNm, "%.*s", LEN_CHART_NM, m_p.zChartNm);
	sprintf(pResult->zChartNm, "%.*s", LEN_CHART_NM, m_p.pChartNow->prevNm);
	...
	*/
	...
	for (int i = 0; i < nLoop; i++)
	{
		...
		if (i == 0)
		{
			g_log.log(LOGTP_SUCC, "<FILLED BASE><%s>[NOW:%.12s](%c)(%10.5f)",
				m_p.zGroupKey, m_p.pChartPrev->Nm, m_p.pChartPrev->gb[0], atof(m_p.pChartPrev->open));
		}
		if (FALSE == m_pShmQ->DataGet((char*)m_p.zGroupKey, (char*)prevChartNm, (char*)&chartUnit[i]))
		{
		...
		g_log.log(LOGTP_SUCC, "<FILLED:%d><%s>[NOW:%.12s](%c)(%10.5f)",
			i, m_p.zGroupKey, chartUnit[i].Nm, chartUnit[i].gb[0], atof(chartUnit[i].open));
	...
	if (nFoundPlus > -1)
	{
		...
		sprintf(m_zMsg, "[OPEN_FILLED_PLUS][%s]"
			"[LASTCHART:%.12s][LASTCHART_OPEN:%.20s]"
			"[MATCHEDCHART:%.12s][MATCHED_OPEN:%.20s]"
			"[SIG GB:%c]",
			m_p.zGroupKey,
			m_p.pChartPrev->Nm, m_p.pChartPrev->open,
			chartMatched.Nm, chartMatched.open,
			pResult->ChartGb[0]
			);
	...
	if (nFoundMinus > -1)
	{
		...
		pResult->ChartGb[0] = CD_OPENSIG_MINUS;
		sprintf(m_zMsg, "[OPEN_FILLED_MINUS][%s]"
			"[LASTCHART:%.12s][LASTCHART_OPEN:%.20s]"
			"[MATCHEDCHART:%.12s][MATCHED_OPEN:%.20s]"
			"[SIG GB:%c]",
			m_p.zGroupKey,
